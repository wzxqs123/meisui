...
window.MobileSelect = (function() {
	...
	MobileSelect.prototype = {
		...
		init: function(config){
		    ...
		    function leftScroll (butEve, nums, scrollEve) {
			    $(butEve).click(function (e) {
	                newCitynum[0] = newCitynum[0] + nums;
	                if (newCitynum[0] <= 0) {
	                    newCitynum[0] = 0;
	                } else if (newCitynum[0] >= 33) {
	                	newCitynum[0] = 33;
	                }
	                var scrollTops = 80 - newCitynum[0] * 40;
	                $(scrollEve).css({
	                    "webkitTransform": 'translate3d(0,' + scrollTops + 'px, 0)',
	                    "transform": 'translate3d(0,' + scrollTops + 'px, 0)'
	                });
	                _this.checkRange(0, newCitynum);
	                console.log(newChildListObject.childs.length);
	                newChildListNum = newChildListObject.childs.length - 1;
	            })
		    }
		    leftScroll(".left-tops", -1, ".selectContainerLeft");
		    leftScroll(".left-bottoms", 1, ".selectContainerLeft");

		    function rightScroll (butEve, nums, scrollEve) {
			    $(butEve).click(function (e) {
	                newCitynum[1] = newCitynum[1] + nums;
	                if (newCitynum[1] <= 0) {
	                    newCitynum[1] = 0;
	                } else if (newCitynum[1] >= newChildListNum) {
	                	newCitynum[1] = newChildListNum;
	                }
	                var scrollTops = 80 - newCitynum[1] * 40;
	                $(scrollEve).css({
	                    "webkitTransform": 'translate3d(0,' + scrollTops + 'px, 0)',
	                    "transform": 'translate3d(0,' + scrollTops + 'px, 0)'
	                });
	                _this.checkRange(1, newCitynum);
	            })
		    }
		    rightScroll(".right-tops", -1, ".selectContainerRight");
		    rightScroll(".right-bottoms", 1, ".selectContainerRight");
		...
		renderWheels: function(wheelsData){
		        ...
		        '<div class="content">'+
		            '<div class="btnBar">'+
		                '<div class="fixWidth">'+
		                    ...
		                    '<div class="title"></div>'+
		                    '<div class="ensure" onclick="btnAction()">选择</div>'+
		                '</div>'+
		            '</div>'+
		            '<div class="panel">'+
		                '<div class="fixWidth">'+
			...
			for(var i=0; i<wheelsData.length; i++){ //列
				tempHTML += '<div class="left-tops position-abs"><i class="position-abs"></i></div><div class="left-bottoms position-abs"><i class="position-abs"></i></div><div class="right-tops position-abs"><i class="position-abs"></i></div><div class="right-bottoms position-abs"><i class="position-abs"></i></div><div class="wheel"><ul class="selectContainer selectContainerLeft">';
				if(_this.jsonType){
			...
			for(var i=0; i<_this.displayJson.length; i++){ //列
				(function (i) {
					...
					if(_this.wheel[i]){
						...
						tempHTML = '<ul class="selectContainer selectContainerRight">';
						for(var j=0; j<_this.displayJson[i].length; j++){ //行
							tempHTML += '<li data-id="'+_this.displayJson[i][j].id+'">'+_this.displayJson[i][j].value+'</li>';
						}
						tempHTML += '</ul>';
